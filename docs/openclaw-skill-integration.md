...
DSA API 默认要求登录认证。Skill 调用时需携带登录后获得的 Cookie，具体方式取决于 openclaw 的 HTTP 工具能力（当前 API 仅支持 Cookie 认证，不支持 Bearer Token）。`ADMIN_AUTH_ENABLED` 仅保留为旧环境兼容字段，设置为 `false` 也不会关闭认证。
